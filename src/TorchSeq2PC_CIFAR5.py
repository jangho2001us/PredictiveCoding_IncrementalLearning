...
def FwdPassPlusCL_CIFAR5(task, taskinfo, model, LossFun, X, Y):
    # Number of layers, counting the input as layer 0
    DepthPlusOne = len(model) + 1
...
def FwdPassPlusCL_CIFAR5_EWC(task, taskinfo, model, LossFun, X, Y):
    # Number of layers, counting the input as layer 0
    DepthPlusOne = len(model) + 1
...
def PCInferCL_CIFAR5(task, taskinfo, model, LossFun, X, Y, ErrType, eta=.1, n=20, vinit=None):
    # Fwd pass (plus return vhat and dLdy)
    # vhat, Loss, dLdy = FwdPassPlus(model, LossFun, X, Y)
    vhat, Loss, dLdy = FwdPassPlusCL_CIFAR5(task, taskinfo, model, LossFun, X, Y)
...
def PCInferCL_CIFAR5_EWC(task, taskinfo, model, LossFun, X, Y, ErrType, eta=.1, n=20, vinit=None):
    # Fwd pass (plus return vhat and dLdy)
    # vhat, Loss, dLdy = FwdPassPlus(model, LossFun, X, Y)
    vhat, Loss, dLdy = FwdPassPlusCL_CIFAR5_EWC(task, taskinfo, model, LossFun, X, Y)
