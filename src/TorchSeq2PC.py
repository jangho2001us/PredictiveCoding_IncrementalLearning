...
def get_target(taskinfo, task, Y):
    # taskinfo = [2, 20, 2, 20, 2, 20, 20, 2, 2, 20]
...
def FwdPassPlusCL(task, taskinfo, model, LossFun, X, Y):
    # Number of layers, counting the input as layer 0
    DepthPlusOne = len(model) + 1
    ...
    # masking with task
    mask = get_mask(taskinfo, task).cuda()

    # calibrate target with task
    Y = get_target(taskinfo, task, Y).cuda()
...
def FwdPassPlusCLEWC(task, taskinfo, model, LossFun, X, Y):
    # Number of layers, counting the input as layer 0
    DepthPlusOne = len(model) + 1
...
def FwdPassPlusIMMMEAN(task, taskinfo, model, LossFun, X, Y):
    # Number of layers, counting the input as layer 0
    DepthPlusOne = len(model) + 1
...
def FwdPassPlusIMMMODE(task, taskinfo, model, LossFun, X, Y):
    # Number of layers, counting the input as layer 0
    DepthPlusOne = len(model) + 1
...
def PCInferCL(task, taskinfo, model, LossFun, X, Y, ErrType, eta=.1, n=20, vinit=None):
    # Fwd pass (plus return vhat and dLdy)
    # vhat, Loss, dLdy = FwdPassPlus(model, LossFun, X, Y)
    vhat, Loss, dLdy = FwdPassPlusCL(task, taskinfo, model, LossFun, X, Y)
...
def PCInferCLEWC(task, taskinfo, model, LossFun, X, Y, ErrType, eta=.1, n=20, vinit=None):
    # Fwd pass (plus return vhat and dLdy)
    # vhat, Loss, dLdy = FwdPassPlus(model, LossFun, X, Y)
    vhat, Loss, dLdy = FwdPassPlusCLEWC(task, taskinfo, model, LossFun, X, Y)
...
def PCInferIMMMEAN(task, taskinfo,  model, LossFun, X, Y, ErrType, eta=.1, n=20, vinit=None):
    # Fwd pass (plus return vhat and dLdy)
    # vhat, Loss, dLdy = FwdPassPlus(model, LossFun, X, Y)
    vhat, Loss, dLdy = FwdPassPlusIMMMEAN(task, taskinfo, model, LossFun, X, Y)
...
def PCInferIMMMODE(task, taskinfo, model, LossFun, X, Y, ErrType, eta=.1, n=20, vinit=None):
    # Fwd pass (plus return vhat and dLdy)
    # vhat, Loss, dLdy = FwdPassPlus(model, LossFun, X, Y)
    vhat, Loss, dLdy = FwdPassPlusIMMMODE(task, taskinfo, model, LossFun, X, Y)
